...
struct HelpView: View {
    ...
    private var wordExportContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            sectionHeader("Usage")
            codeBlock("SpindriftCLI convert <input.pdf> [output.docx]")
            ...
            codeBlock("SpindriftCLI convert report.pdf")

            Text("Convert pages 1\u{2013}3 (0-indexed):").bold()
            codeBlock("SpindriftCLI convert report.pdf --start 0 --end 3")

            Text("Convert specific pages:").bold()
            codeBlock("SpindriftCLI convert report.pdf output.docx --pages 0,2,4")
        }
    }
    ...
    private var tableExtractionContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            sectionHeader("Usage")
            codeBlock("SpindriftCLI tables <input.pdf> [output.xlsx]")
            ...
            codeBlock("SpindriftCLI tables data.pdf")

            Text("Use a specific detection method:").bold()
            codeBlock("SpindriftCLI tables data.pdf --method lines")

            Text("Extract from specific pages:").bold()
            codeBlock("SpindriftCLI tables data.pdf results.xlsx --pages 1,3,5")
        }
    }
    ...
    private var autoTableDetectionContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("When extracting tables, Spindrift offers four detection methods. You can choose a method from the detection method picker in the table toolbar, or pass --method on the command line.")
