...
final class AerialistDocument: ReferenceFileDocument, @unchecked Sendable {
    typealias Snapshot = DocumentSnapshot

    /// Marker userName on the hidden annotation that stores sidecar JSON.
    static let sidecarMarker = "com.aerialist.sidecar"

    /// Prefix for all Aerialist-managed annotations (comments, stamps, text boxes).
    static let annotationPrefix = "aerialist:"
    ...
    /// Build a userName tag for a managed annotation: "aerialist:type:UUID"
    static func annotationTag(type: String, id: UUID) -> String {
        "\(annotationPrefix)\(type):\(id.uuidString)"
    }

    /// Parse a userName tag. Returns (type, UUID) or nil if not an Aerialist tag.
    ...
    /// After this, the PDF pages have no Aerialist-managed annotations — the coordinator recreates them.
    ///
    ...
    private static func reconcileAndStrip(pdf: PDFDocument, sidecar: inout SidecarModel) {
        ...
        for pageIndex in 0..<pdf.pageCount {
            ...
            for annotation in page.annotations {
                ...
                // Check if this is a tagged Aerialist annotation
